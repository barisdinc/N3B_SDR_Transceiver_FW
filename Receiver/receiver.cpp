...
int receiver::adc_dma_pong;
dma_channel_config receiver::ping_cfg;
dma_channel_config receiver::pong_cfg;
uint16_t receiver::ping_samples[adc_block_size];
uint16_t receiver::pong_samples[adc_block_size];
...
dma_channel_config receiver::capture_cfg;
static uint32_t lcount = 0;
void receiver::dma_handler() {
    ...
    if (lcount++ >= 0x000000FF) {printf("m"); lcount=0;}
...
void receiver::update_status()
{
   if(sem_try_acquire(&settings_semaphore))
   {
     ...
     //update status
    //  status.signal_strength_dBm = receiver_dsp_inst.get_signal_strength_dBm();
...
//BARIS      nco_frequency_Hz = nco_set_frequency(pio, sm, tuned_frequency_Hz, system_clock_rate);
      ...
      receiver_dsp_inst.set_frequency_offset_Hz(offset_frequency_Hz);

      //apply CW sidetone
      receiver_dsp_inst.set_cw_sidetone_Hz(settings_to_apply.cw_sidetone_Hz);

      //apply gain calibration
      receiver_dsp_inst.set_gain_cal_dB(settings_to_apply.gain_cal);

      //apply AGC speed
      receiver_dsp_inst.set_agc_speed(settings_to_apply.agc_speed);

      //apply Automatic Notch Filter
      receiver_dsp_inst.set_auto_notch(settings_to_apply.enable_auto_notch);

      //apply mode
      receiver_dsp_inst.set_mode(settings_to_apply.mode, settings_to_apply.bandwidth);
      ...
      //apply deemphasis
      receiver_dsp_inst.set_deemphasis(settings_to_apply.deemphasis);

      //apply squelch
      receiver_dsp_inst.set_squelch(settings_to_apply.squelch);

      //apply swap iq
      receiver_dsp_inst.set_swap_iq(settings_to_apply.swap_iq);

      //apply iq imbalance correction
      receiver_dsp_inst.set_iq_correction(settings_to_apply.iq_correction);
...
void receiver::get_spectrum(uint8_t spectrum[], uint8_t &dB10)
{
 ...
 * All required settings and pre-initializations (e.g. gpio mappings) will be here
*/
receiver::receiver(rx_settings & settings_to_apply, rx_status & status) : settings_to_apply(settings_to_apply), status(status)
{
