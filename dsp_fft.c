...
int16_t XI_fft_buf[FFT_SIZE] __attribute__((aligned(4))); 					// Buffer for spectrum display (real part of FFT)
...
int16_t* get_fft_buffer_address()
{
	return &XI_fft_buf[0];
}
...
bool __not_in_flash_func(rx)(void) 
{
	int b;
	int i;
	int16_t *ip, *qp, *ap, *xip, *xqp, *xifftp;
	int16_t peak;
	...
	for (i=0; i<FFT_SIZE; i++) *xifftp++ = *xip++;							// copy spectrum buffer
	...
	case MODE_USB:
		...
		for (i=1; i<BIN_3000; i++)
		{
			...
			//ili9341_draw_line(i,150,i,220-(XI_buf[i]>>9), ILI9341_BLUE);
			//printf("%d\n",abs(XI_buf[i])>>8);
		}
		//printf("-----\n");
		// Bandpass DSB (2x USB)
