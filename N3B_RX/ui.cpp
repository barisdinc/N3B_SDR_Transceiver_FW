...
//------------------------------------------------------------
  ...
  #define PIN_CS   12 //13
  #define PIN_SCK  10 //14
  #define PIN_MOSI 11 //15 
  #define PIN_DC   4 //11
  #define PIN_RST  5 //10
  #define SPI_PORT spi1
  //spi_init(SPI_PORT, 62500000);
  spi_init(SPI_PORT, 40000000);
  // spi_init(SPI_PORT, 90000000);
...
void ui::configure_display(uint8_t settings, bool invert_colours)
{
    if(settings == 0)
    {
      enabled = false;
...
uint16_t ui::dBm_to_px(float power_dBm, int16_t px) {
...
int ui::dBm_to_S2(float power_dBm) {
  int power_s = floorf((power_dBm-S0)/6.0f);
  if(power_dBm >= S9) power_s = floorf((power_dBm-S9)/10.0f)+9;
  if(power_s < 0) power_s = 0;
  if(power_s > 12) power_s = 12;
...
void ui::update_spectrum(rx &receiver, rx_settings &settings, rx_status &status, uint8_t ilispectrum[], uint8_t dB10)
{
    ...
    if(FSM_state == update_waterfall)
    {
      ...
      {
        waterfall_buffer[top_row][col] = ilispectrum[col];
      }
