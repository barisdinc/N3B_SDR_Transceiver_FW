...
void TestRandomLabels(screen_control_t *p_screen)
{
    ...
    TftPutTextLabel(p_screen, "A", x, y, false);
...
int main() 
{
    ...
    (pin 38)  GND        Ground.
16/GP12 (pin 07)  CS         LCD chip select signal, low level enable.
7/GP5    (pin 11)  RESET      LCD reset signal, low level reset.
6/GP4    (pin 12)  DC/RS      LCD register / data selection signal; high level: register.
15/GP11    (pin 10)  SDI(MOSI)  SPI bus write data signal.
14/GP10    (pin 09)  SCK        SPI bus clock signal.
    (pin 36)  LED        Backlight control.
11/GP8    (pin 06)  SDO(MISO)  SPI bus read data signal. Hasn't been used so far in here.
...
void ILI9341_Init(ili9341_config_t *pconfig, spi_inst_t *pspi_port, 
                    int spi_clock_freq, 
                    int gpio_MISO, 6/GP4 -> 11/GP8 
                    int gpio_CS,   7/GP5 -> 16/GP12
                    int gpio_SCK,  9/GP6 -> 14/GP10
                    int gpio_MOSI,10/GP7 -> 15/GP11 
                    int gpio_RS,  11/GP8 ->  7/GP5
                    int gpio_DC   12/GP9 ->  6/GP4
                    );
    */

    ili9341_config_t ili9341_hw_config;
    sScreen.mpHWConfig = &ili9341_hw_config;
    ILI9341_Init(sScreen.mpHWConfig, spi1, 90 * MHz, 8, 12, 10, 11, 5, 4);
