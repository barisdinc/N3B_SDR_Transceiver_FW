...
void adf4360_evaluate(void)
{
  // return;
    ...
    uint32_t new_sat_rx_freq = (2*(vfo[0].freq + RX_SHFT_FREQ))/1000;  //TODO: Change 2* to a declare
    ...
    unsigned int steps = round(new_sat_rx_freq / PFD)+1 ; //vfo[0].freq / PFD;
    unsigned long b_counter =  round(steps / (8 * (1 << vfo[0].ps))); // 00 -> 8  01 -> 16 10 -> 32
...
void adf4360_init(void)
{
  ...
  vfo[0].cpl   = 2;    // core power level 00(0):5mA 01(1):10mA 10(2):15mA 11(3):20mA
	vfo[0].div2i = 1;//0;    // prescaler input divide-by 2 select 0:fundamental 1:divided by 2
